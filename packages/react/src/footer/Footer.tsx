...
type FooterMeta = {
    ...
    text?: string;
    /** Re-authored meta content (raw govuk `meta.html`). */
    content?: ReactNode;
};
type FooterLicence = {
    text?: string;
    /** Re-authored licence content (raw govuk `contentLicence.html`). */
    content?: ReactNode;
} | null;
type FooterCopyright = {
    text?: string;
    /** Re-authored copyright content (raw govuk `copyright.html`). */
    content?: ReactNode;
};
...
export default function Footer({
    ...
    const classes = ['govuk-footer', className ?? ''].filter(Boolean).join(' ');
                    ...
                    <>
                        <div className="govuk-footer__navigation">
                            {navigation.map((nav, i) => (
                                <div
                                    key={i}
                                    className={`govuk-footer__section govuk-grid-column-${nav.width ?? 'full'}`}
                                >
                                    ...
                                    {nav.items && nav.items.length > 0 && (
                                        ...
                                        >
                                            {nav.items
                                                ...
                                                .map((item, j) => (
                                                    <li key={j} className="govuk-footer__list-item">
                                                        <a
                        ...
                        {contentLicence !== null && (
                            <>
                                ...
                                <span className="govuk-footer__licence-description">
                                    {contentLicence && (contentLicence.content || contentLicence.text) ? (
                                        contentLicence.content ?? contentLicence.text
                                    ) : (
                                        <>
