...
const htmlContent: Record<string, JSX.Element> = {
    "with text as html": (
        <>
            ...
            <p class="govuk-body">The following 4 files are available</p>
    ...
    "with lots of content": (
        <>
            <h3 class="govuk-notification-banner__heading">Check if you need to apply the reverse charge to this application</h3>
            <p class="govuk-body">
...
export function renderNotificationBanner(data: NotificationBannerData): JSX.Element {
    return (
        <NotificationBanner success={data.success ? "true" : undefined}>
            ...
            <NotificationBannerContent>
                <Show
                    when={data.html !== undefined}
                    fallback={<NotificationBannerHeading>{data.text}</NotificationBannerHeading>}
                >
