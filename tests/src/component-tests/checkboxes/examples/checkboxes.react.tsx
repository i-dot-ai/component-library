...
function renderItem(item: CheckboxItemData): ReactNode {
    if ("divider" in item) {
    ...
    return (
        <>
            <CheckboxItem>
                ...
                />
                <CheckboxLabel htmlFor={item.id}>{item.text}</CheckboxLabel>
                {item.hintId && (
...
export function renderCheckboxes(data: CheckboxesData): ReactNode {
    const checkboxes = (
        <Checkboxes
